...
| `channel` | `stable` | `stable` (newest release marked latest — unique `stable-*` tags per CI run), `bleeding-edge` (newest `bleeding-edge-*` prerelease), or a pin tag like `v0.0.1` (immutable Release; the tag **is** the version — no `version.txt`). |
...
Downloads are verified against `SHA256SUMS` on the release (Linux `.tar.gz` or Windows `.zip` for the host RID). Rolling channels query the GitHub Releases API to resolve the current unique tag, then read `version.txt`; pinned `v*` channels compare the running binary’s informational version to the pin tag instead. The live binary is replaced via
